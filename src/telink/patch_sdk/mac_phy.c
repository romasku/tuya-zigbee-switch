...
// --- [PATCH] 1. INCLUDE GPIO DRIVER ---
#include "../../../proj/drivers/drv_gpio.h"

// --- [PATCH] 2. DEFINE PA/LNA PINS ---
#define GPIO_PA_TX_PIN   GPIO_PC4
#define GPIO_PA_RX_PIN   GPIO_PC3

// --- [PATCH] 3. POWER SETTINGS ---
// Use -6dBm input to avoid saturation (Raw hex 0x21 if symbol missing)
#ifndef RF_POWER_N5p95dBm
#define RF_POWER_N5p95dBm 0x21
#endif

// --- [PATCH] 4. PA HELPERS WITH TIMING ---
#define PA_RX_ON()   do{ \
                        drv_gpio_write(GPIO_PA_TX_PIN, 0); \
                        drv_gpio_write(GPIO_PA_RX_PIN, 1); \
                        WaitUs(5); \
                     }while(0)

#define PA_TX_ON()   do{ \
                        drv_gpio_write(GPIO_PA_RX_PIN, 0); \
                        drv_gpio_write(GPIO_PA_TX_PIN, 1); \
                        WaitUs(5); \
                     }while(0)
...
// [PATCH] Force PA Flag Enabled by default so rf_setTxPower uses our logic
u8 fPaEn = 1;
...
void rf_reset(void)
{
    // [PATCH] Initialize PA Pins
    drv_gpio_func_set(GPIO_PA_TX_PIN);
    drv_gpio_output_en(GPIO_PA_TX_PIN, 1);
    drv_gpio_write(GPIO_PA_TX_PIN, 0);

    drv_gpio_func_set(GPIO_PA_RX_PIN);
    drv_gpio_output_en(GPIO_PA_RX_PIN, 1);
    drv_gpio_write(GPIO_PA_RX_PIN, 0);

    // [PATCH] Force PA Flag to 1
    fPaEn = 1;

    rf_setTrxState(RF_STATE_OFF);

    rf_setTxPower(g_zb_txPowerSet); // Will call patched logic
...
{
    if (fPaEn) {
        // [PATCH] Force Low Power (-6dBm) to avoid PA Saturation
        ZB_RADIO_TX_POWER_SET(0x21);
    } else {
...
{
// [PATCH] Logic moved to PA_OFF macro, but we can leave this for safety
#if PA_ENABLE
    if (fPaEn) {
