...
#define SETUP_ATTR(attr_index, attr_id, attr_type, flag_val, attr_data) \
        SETUP_ATTR_FOR_TABLE(cluster->attr_infos, attr_index, attr_id, attr_type, flag_val, attr_data)
...
#define DEF_STR(string, name)              struct { uint8_t len; char str[sizeof(string) - 1]; } const name = { sizeof(string) - 1, string }
#define DEF_STR_NON_CONST(string, name)    struct { uint8_t len; char str[sizeof(string) - 1]; } name = { sizeof(string) - 1, string }
