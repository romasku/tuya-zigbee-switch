...
1.  **Permit join** when it's done (LED blinks if mapped correctly)
2.  **Interview** the device **`i`**  
  ⤷ option missing from ZHA, remove and re-pair if needed  
1.    **Reconfigure** the device **`🗘`** 
2.    Be aware of [## Version update](#version-update) steps
3.    Read [faq.md](./faq.md) before reaching out for support
